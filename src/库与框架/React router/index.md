...
作为root组件导出，在项目中包裹在App组件外部

```tsx
// react-router/lib/components.tsx
...
但是，现在一般不直接在项目里使用router了。在node环境，用StaticRouter，限制了一些navigator方法；在浏览器环境，用BrowerRouter
...
- LocationContext 的结构和赋值

  ```typescript
  // react-router/packages/react-router/lib/context.ts
