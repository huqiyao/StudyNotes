...
## js的各种位置， 比如 clientHeight,scrollHeight,offsetHeight ,以及 scrollTop, offsetTop,clientTop
的区别  
...
- computed：计算属性，当依赖的数据更新时显示新的计算结果（结果会缓存）
  ...
  场景：设计初衷是为了解决在模板中填写复杂的计算表达式的问题，如果一个数据依赖于其他数据又需要重复取值就使用computed
...
Vue的一个内置组件，作用是使得被包含的组件保留状态，不被重新渲染
